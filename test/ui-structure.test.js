...
const editorViewSource = fs.readFileSync(require.resolve('../src/EditorView.js'), 'utf8');
...
assert.match(editorViewSource, /lineNumbers\.scrollTop = this\.elements\.editor\.scrollTop/, 'line number gutter must follow editor scrolling');
assert.match(editorViewSource, /CFGFILE\|\(\?:CRYPTED\)\?BINFILE\|\(\?:CRYPTED\)\?B64FILE/, 'editor section navigation must recognize export file markers');
assert.match(editorViewSource, /button\.addEventListener\("click", \(\) => this\.onJumpRequest\(section\.line\)\)/, 'section markers must navigate to their source line');
...
assert.match(html, /this\.editorTheme = this\.editorTheme === 'console'/, 'the expert editor needs an independent console/light theme toggle');
...
assert.match(html, /src\/EditorView\.js\?v=\d{8}-\d+/, 'editor view needs a deployment cache key');
assert.equal(html.includes('EditorView API v1'), true, 'editor view compatibility guard is missing');
...
assert.match(html, /src\/FilesView\.js\?v=\d{8}-\d+/, 'files view needs a deployment cache key');
assert.equal(html.includes('FilesView API v1'), true, 'files view compatibility guard is missing');
assert.match(filesViewSource, /renderEmbeddedFiles\(data\)/, 'embedded file DOM rendering belongs in the files view');
assert.match(filesViewSource, /renderPhonebooks\(data, queryValue\)/, 'phonebook DOM rendering belongs in the files view');
...
assert.match(filesViewSource, /onJumpToLine\(line\)/, 'phonebooks must link back to their source block');
assert.equal(filesViewSource.includes('Dekodierte XML-Quelldatei anzeigen'), true, 'phonebook cards must expose their decoded XML source on demand');
assert.match(filesViewSource, /this\.elements\.xml\.appendChild\(details\)/, 'decoded phonebook XML must render below the structured phonebooks');
assert.match(filesViewSource, /file\.type === "B64FILE"/, 'generic file viewer must only expose decoded plain B64 files');
assert.match(filesViewSource, /file\.name\.toLowerCase\(\) !== "phonebook"/, 'structured phonebooks must not be duplicated in the generic file viewer');
assert.match(filesViewSource, /this\.createFilePreview\(file\)/, 'generic files must use the safe decoded preview service');
assert.match(filesViewSource, /createSourceButton\(file\.line\)/, 'generic embedded files must link back to their source block');
